semimetric.interv <- function(data1, data2, interv=c(3,7), range.grid=c(1,20), nknot=6)
{
...
aa <- interv[1]
bb <- interv[2]
if ((aa<a) | (bb>b)) stop("interv must be cointained in range.grid")
order.Bspline <- 0 + 3
...
x.gauss <- 0.5 * ((bb + aa) + (bb - aa) * point.gauss)
lx.gauss <- length(x.gauss)
Bspline.deriv <- splineDesign(delta, x.gauss, order.Bspline, rep(0, lx.gauss))
H <- t(Bspline.deriv) %*% (Bspline.deriv * (weight.gauss * 0.5 * (bb - aa)))
