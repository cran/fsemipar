...
if(is.null(newdata.x)|is.null(newdata.z)){
	...
	if (option==2) {
		aux2 <-fsim.kernel.fit.fixedtheta(y=y.new,x=object$x, theta=object$theta.est, kind.of.kernel=object$kind.of.kernel,
...
plot.sfplsim.kernel<-function(x,cex.axis=1.5,cex.lab=1.5,cex=2,col=1,cex.main=1.5,...)
{
